...
const DashboardSingleCategory = ({
  ...
  const deleteCategory = async () => {
    ...
    // отправка запроса API на удаление категории
  ...
  const updateCategory = async () => {
    if (categoryInput.name.length > 0) {
      ...
      // отправка запроса API для обновления категории
      fetch(`http://localhost:3001/api/categories/${id}`, requestOptions)
        .then((response) => {
  ...
  useEffect(() => {
    // отправка запроса API для получения единой категории
  ...
  return (
    <div className="bg-white flex justify-start max-w-screen-2xl mx-auto xl:h-full max-xl:flex-col max-xl:gap-y-5">
      ...
      <div className="flex flex-col gap-y-7 xl:pl-5 max-xl:px-5 w-full">
        <h1 className="text-3xl font-semibold">Подробная информация о категории</h1>
        <div>
