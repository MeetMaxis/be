...
export const useProductStore = create<State & Actions>()(
  persist(
    ...
    {
      name: "products-storage", // название товара, находящегося на складе (должно быть уникальным)
      storage: createJSONStorage(() => sessionStorage), // по умолчанию используется 'localStorage' (необязательно) 
    }
  )
);
