...
const CheckoutPage = () => {
  ...
  const makePurchase = async () => {
    if (
      ...
      if (!isValidNameOrLastname(checkoutForm.cardName)) {
        toast.error("Вы ввели неверный формат названия карты");
        return;
      }
      ...
      if (!isValidCreditCardExpirationDate(checkoutForm.expirationDate)) {
        toast.error(
          "Вы ввели неверный формат даты истечения срока действия кредитной карты"
        );
        return;
      }

      if (!isValidCreditCardCVVOrCVC(checkoutForm.cvc)) {
        toast.error("Вы ввели неверный формат для кредитной карты CVC или CVVV");
        return;
      }

      // отправка запроса API для создания заказа
      const response = fetch("http://localhost:3001/api/orders", {
        ...
        .then((data) => {
          ...
          // для каждого продукта в заказе мы вызываем функцию addOrderProduct, которая добавляет поля в таблицу customer_order_product
        ...
        .then(() => {
          ...
          clearCart();
          toast.success("Успешно созданный заказ");
          setTimeout(() => {
            router.push("/");
          }, 1000);
        });
  ...
  const addOrderProduct = async (
    ...
    // отправка POST-запроса API для таблицы customer_order_product, которая определяет соотношение "многие ко многим" для заказа и продукта
    const response = await fetch("http://localhost:3001/api/order-product", {
      method: "POST", // либо 'PUT'
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
  ...
  return (
    <div className="bg-white">
      ...
      <main className="relative mx-auto grid max-w-screen-2xl grid-cols-1 gap-x-16 lg:grid-cols-2 lg:px-8 xl:gap-x-48">
        <h1 className="sr-only">Информация о заказе</h1>
          ...
          <div className="mx-auto max-w-lg lg:max-w-none">
            ...
            <dl className="hidden space-y-6 border-t border-gray-200 pt-6 text-sm font-medium text-gray-900 lg:block">
              <div className="flex items-center justify-between">
                <dt className="text-gray-600">Промежуточный итог</dt>
                <dd>${total}</dd>
              </div>
              ...
              <div className="flex items-center justify-between">
                <dt className="text-gray-600">Налоги</dt>
                <dd>${total / 5}</dd>
              </div>
