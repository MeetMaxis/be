...
const SingleProductPage = async ({ params }: SingleProductPageProps) => {
  // завершение запроса API для одного продукта с помощью заданного модуля product slug
  const data = await fetch(
    `http://localhost:3001/api/slugs/${params.productSlug}`
  );
  const product = await data.json();

  // отправка запроса API для получения более чем 1 изображения продукта, если оно существует
